        ...
        if ret:
            ...
            for i in indices:
                ...
                if not self.detect_single_object or (self.detect_single_object and label == self.desired_object):
                    ...
                    # Check if object is somewhat in the center box
                    in_center = (square_top_left[0] < object_center_x < square_bottom_right[0] and
                                 square_top_left[1] < object_center_y < square_bottom_right[1])

                    if in_center:
                        status = "In center"
                    else:
                        # Determine direction
                        dx = center[0] - object_center_x
                        dy = center[1] - object_center_y
                        ...
                        status = f"Move camera {direction}"

                    # Display object info and status
                    info_text = f"{label}: {confidences[i]:.2f} - {status}"
